...
def test_addition():
    source = "1 + 1"
    assert parse(source) == "(+ 1 1)"


def test_addition_and_subtraction():
    source = "1 + 1 - 1"
    assert parse(source) == "(- (+ 1 1) 1)"


def test_multiplication_and_division():
    source = "2 * 3 / 4"
    assert parse(source) == "(/ (* 2 3) 4)"


def test_basic_primaries():
    source = "123"
    assert parse(source) == "123"
...
def test_expression_primary():
    source = "(123 - (456 / 789))"
    assert parse(source) == "(group (- 123 (group (/ 456 789))))"
...
def test_factor():
    source = "-123 + 1"
    assert parse(source) == "(+ (- 123) 1)"


def test_equality():
    source = "2 != 2"
    assert parse(source) == "(!= 2 2)"
...
def test_complex_expression():
    ...
    assert parse(source) == "(== (>= (* (group (+ 1 (/ 2 2))) (- 1)) 0) False)"
